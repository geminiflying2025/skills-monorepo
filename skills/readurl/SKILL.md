...
如果用户只给链接、没有额外说明，默认输出总结：先运行 `read_link.py` 生成本地语料包，再阅读 `corpus.md`、`result.json` 和必要的媒体产物，最后直接给用户一份简洁总结。不要只回复“已生成文件”或只给产物路径。

默认总结应包含：

- 一句话主题
- 关键要点
- 重要证据来源，例如正文、字幕、音频转写、图片 OCR、关键帧或平台元数据
- 产物路径摘要
- 失败或跳过的阶段

如果语料不足，仍要输出结论边界，例如“语料不足，无法可靠总结”，并说明缺少什么；不要补编内容。
